...
class Heap{
  public:
    ...
    void insert(int val) {
      ...
      while(index > 1) {
        int parentIndex = index/2; // parent index find krne ka tarrika hai kisi bhi elemt ka parendt index n/2 se niklega
    ...
    int deleteFromHeap() {
      int answer = arr[1];// arr 1 means mein root node pr hu 
      //replacement
      arr[1] = arr[size];//mtlb abhi root mein hun
      //last element ko delete uski original position se
      size--;
      int index = 1;
      while(index < size ) {
          int leftIndex = 2*index;//left node ke liye
          int rightIndex = 2*index+1;//right side ke liye
